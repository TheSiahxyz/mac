...
function Thumbnailer:update_state()
	...
	if
		has_video
		and self.state.thumbnail_delta ~= nil
		and self.state.thumbnail_size ~= nil
		and self.state.thumbnail_count > 0
	then
...
function Thumbnailer:start_worker_jobs()
	...
	local worker_list = {}
	for worker_name in pairs(self.workers) do
		table.insert(worker_list, worker_name)
	end
	...
	if worker_count == 0 then
		...
		if mp.get_time() > self.worker_register_timeout then
			...
			self.worker_wait_timer = mp.add_timeout(wait_time, function()
				self:start_worker_jobs()
			end)
		end
	else
...
function display_thumbnail(pos, value, ass)
	...
	if not ((duration == nil) or (value == nil)) then
		...
		local pad = {
			l = thumbnailer_options.pad_left,
			r = thumbnailer_options.pad_right,
			t = thumbnailer_options.pad_top,
			b = thumbnailer_options.pad_bot,
		}
		...
		ass:append(
			("{\\bord0\\1c&H%s&\\1a&H%X&}"):format(
				thumbnailer_options.background_color,
		...
		if display_progress then
			...
			if closest_index ~= nil then
				...
				ass:draw_start(2)
				ass:rect_cw(
					(closest_index - 1) * block_w,
					0,
					math.min(block_max_x, closest_index * block_w),
					framegraph_h
				)
				ass:draw_stop()
			end
		...
		if thumb_path then
			...
			if
				not (
					...
					and osc_thumb_state.last_x == thumb_x
					and osc_thumb_state.last_y == thumb_y
				)
			then
				local overlay_add_args = {
					"overlay-add",
					osc_thumb_state.overlay_id,
					thumb_x,
					thumb_y,
					thumb_path,
					0,
					...
					if (marker > element.slider.min.value) and (marker < element.slider.max.value) then
						...
						if slider_lo.gap > 1 then -- draw triangles
							...
							if slider_lo.nibbles_bottom then
								static_ass:move_to(s - (a / 2), elem_geo.h - slider_lo.border)
							...
							--top
							if slider_lo.nibbles_top then
								static_ass:rect_cw(s - 1, slider_lo.border, s + 1, slider_lo.border + slider_lo.gap)
							end
...
function render_elements(master_ass)
	for n = 1, #elements do
		...
		style_ass:append(string.format("{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}", ar[1], ar[2], ar[3], ar[4]))
		...
		if element.type == "slider" then
			...
			if not (pos == nil) then
				...
				elseif slider_lo.stype == "knob" then
					...
					elem_ass:round_rect_cw(xp - innerH / 2, foV, xp + innerH / 2, foV + innerH, innerH / 2.0)
				end
			end
			...
			if not (element.slider.tooltipF == nil) then
				if mouse_hit(element) then
					...
					elem_ass:append(string.format("{\\1a&H%X&\\2a&H%X&\\3a&H%X&\\4a&H%X&}", ar[1], ar[2], ar[3], ar[4]))
...
function render_message(ass)
	if
		not (state.message_timeout == nil)
		and not (state.message_text == nil)
		and state.message_timeout > mp.get_time()
	then
...
layouts["box"] = function()
	...
	local posX = math.floor(get_align(user_opts.halign, osc_param.playresx, osc_geo.w, 0))
	local posY = math.floor(get_align(user_opts.valign, osc_param.playresy, osc_geo.h, 0))
	...
	if user_opts.valign > 0 then
		-- deadzone above OSC
		sh_area_y0 = get_align(-1 + (2 * user_opts.deadzonesize), posY - (osc_geo.h / 2), 0, 0)
		sh_area_y1 = osc_param.playresy
	else
		-- deadzone below OSC
		sh_area_y0 = 0
		sh_area_y1 = (posY + (osc_geo.h / 2))
			+ get_align(1 - (2 * user_opts.deadzonesize), osc_param.playresy - (posY + (osc_geo.h / 2)), 0, 0)
	end
	...
	lo.geometry = { x = (posX - pos_offsetX), y = titlerowY, an = 7, w = 12, h = 12 }
	lo.style = osc_styles.topButtons

	lo = add_layout("pl_next")
	lo.geometry = { x = (posX + pos_offsetX), y = titlerowY, an = 9, w = 12, h = 12 }
	lo.style = osc_styles.topButtons
	...
	lo.geometry = { x = posX, y = bigbtnrowY, an = 5, w = 40, h = 40 }
	lo.style = osc_styles.bigButtons

	lo = add_layout("skipback")
	lo.geometry = { x = posX - bigbtndist, y = bigbtnrowY, an = 5, w = 40, h = 40 }
	lo.style = osc_styles.bigButtons
	...
	lo.geometry = { x = posX - (bigbtndist * 2), y = bigbtnrowY, an = 5, w = 40, h = 40 }
	lo.style = osc_styles.bigButtons
	...
	lo.geometry = { x = posX - pos_offsetX, y = bigbtnrowY, an = 1, w = 70, h = 18 }
	lo.style = osc_styles.smallButtonsL

	lo = add_layout("cy_sub")
	lo.geometry = { x = posX - pos_offsetX, y = bigbtnrowY, an = 7, w = 70, h = 18 }
	lo.style = osc_styles.smallButtonsL

	lo = add_layout("tog_fs")
	lo.geometry = { x = posX + pos_offsetX - 25, y = bigbtnrowY, an = 4, w = 25, h = 25 }
	lo.style = osc_styles.smallButtonsR

	lo = add_layout("volume")
	lo.geometry = { x = posX + pos_offsetX - (25 * 2) - osc_geo.p, y = bigbtnrowY, an = 4, w = 25, h = 25 }
	lo.style = osc_styles.smallButtonsR
	...
	lo.geometry = { x = posX, y = posY + pos_offsetY - 22, an = 2, w = pos_offsetX * 2, h = 15 }
	...
	lo.geometry = { x = posX - pos_offsetX, y = bottomrowY, an = 4, w = 110, h = 18 }
	lo.style = osc_styles.timecodes

	lo = add_layout("tc_right")
	lo.geometry = { x = posX + pos_offsetX, y = bottomrowY, an = 6, w = 110, h = 18 }
	lo.style = osc_styles.timecodes

	lo = add_layout("cache")
	lo.geometry = { x = posX, y = bottomrowY, an = 5, w = 110, h = 18 }
	lo.style = osc_styles.timecodes
end
...
layouts["slimbox"] = function()
	...
	lo = add_layout("tc_left")
	lo.geometry = { x = posX - (inner_w / 2) + osc_geo.r, y = posY + 1, an = 7, w = tc_w, h = ele_h }
	lo.style = styles.timecodes
	lo.alpha[3] = user_opts.boxalpha

	lo = add_layout("tc_right")
	lo.geometry = { x = posX + (inner_w / 2) - osc_geo.r, y = posY + 1, an = 9, w = tc_w, h = ele_h }
	lo.style = styles.timecodes
	lo.alpha[3] = user_opts.boxalpha
	...
	lo = add_layout("cache")
	lo.geometry = { x = posX, y = posY + 1, an = 8, w = tc_w, h = ele_h }
	lo.style = styles.timecodes
	lo.alpha[3] = user_opts.boxalpha
end
...
layouts["bottombar"] = function()
	...
	local sh_area_y0, sh_area_y1
	sh_area_y0 = get_align(-1 + (2 * user_opts.deadzonesize), osc_geo.y - (osc_geo.h / 2), 0, 0)
	...
	lo.geometry = geo
	lo.style =
		string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.vidtitleBar, geo.x, geo.y - geo.h, geo.w, geo.y + geo.h)
...
layouts["topbar"] = function()
	...
	sh_area_y1 = (osc_geo.y + (osc_geo.h / 2))
		+ get_align(1 - (2 * user_opts.deadzonesize), osc_param.playresy - (osc_geo.y + (osc_geo.h / 2)), 0, 0)
	add_area("showhide", 0, sh_area_y0, osc_param.playresx, sh_area_y1)
...
function validate_user_opts()
	...
	if user_opts.seekbarstyle ~= "slider" and user_opts.seekbarstyle ~= "bar" and user_opts.seekbarstyle ~= "knob" then
...
function osc_init()
	...
	ne.content = function()
		...
		return ("\238\132\134" .. osc_styles.smallButtonsLlabel .. " " .. aid .. "/" .. #tracks_osc.audio)
	end
	ne.eventresponder["mbtn_left_up"] = function()
		set_track("audio", 1)
	end
	...
	ne.content = function()
		...
		return ("\238\132\135" .. osc_styles.smallButtonsLlabel .. " " .. sid .. "/" .. #tracks_osc.sub)
	end
	ne.eventresponder["mbtn_left_up"] = function()
		set_track("sub", 1)
	end
	...
	ne.eventresponder["mouse_move"] = --keyframe seeking when mouse is dragged
		function(element)
			...
			if (element.state.lastseek == nil) or not (element.state.lastseek == seekto) then
				mp.commandv("seek", seekto, "absolute-percent", "keyframes")
	...
	ne.content = function()
		...
		if (is_network and dmx_cache) or show_cache then
			...
			return string.format(
				...
				(show_cache and utils.format_bytes_humanized(cache_used) or "")
			)
		else
			return ""
		end
	...
	ne.content = function()
		...
		local volicon = { "\238\132\139", "\238\132\140", "\238\132\141", "\238\132\142" }
...
function render()
	...
	-- autohide
	if
		not (state.showtime == nil)
		and (user_opts.hidetimeout >= 0)
		and (state.showtime + (user_opts.hidetimeout / 1000) < now)
		and (state.active_element == nil)
		and not mouse_over_osc
	then
		hide_osc()
	end
...
function tick()
	...
	if state.idle then
		...
		ass:append(
			"{\\rDefault\\an7\\c&H430142&\\1a&H00&\\bord0\\shad0\\p6}m 1605 828 b 1605 1175 1324 1456 977 1456 631 1456 349 1175 349 828 349 482 631 200 977 200 1324 200 1605 482 1605 828{\\p0}"
		)
		ass:new_event()
		ass:pos(icon_x, icon_y)
		ass:append(
			"{\\rDefault\\an7\\c&HDDDBDD&\\1a&H00&\\bord0\\shad0\\p6}m 1296 910 b 1296 1131 1117 1310 897 1310 676 1310 497 1131 497 910 497 689 676 511 897 511 1117 511 1296 689 1296 910{\\p0}"
		)
		ass:new_event()
		ass:pos(icon_x, icon_y)
		ass:append(
			"{\\rDefault\\an7\\c&H691F69&\\1a&H00&\\bord0\\shad0\\p6}m 762 1113 l 762 708 b 881 776 1000 843 1119 911 1000 978 881 1046 762 1113{\\p0}"
		)
		ass:new_event()
		ass:pos(icon_x, icon_y)
		ass:append(
			"{\\rDefault\\an7\\c&H682167&\\1a&H00&\\bord0\\shad0\\p6}m 925 42 b 463 42 87 418 87 880 87 1343 463 1718 925 1718 1388 1718 1763 1343 1763 880 1763 418 1388 42 925 42 m 925 42 m 977 200 b 1324 200 1605 482 1605 828 1605 1175 1324 1456 977 1456 631 1456 349 1175 349 828 349 482 631 200 977 200{\\p0}"
		)
		ass:new_event()
		ass:pos(icon_x, icon_y)
		ass:append(
			"{\\rDefault\\an7\\c&H753074&\\1a&H00&\\bord0\\shad0\\p6}m 977 198 b 630 198 348 480 348 828 348 1176 630 1458 977 1458 1325 1458 1607 1176 1607 828 1607 480 1325 198 977 198 m 977 198 m 977 202 b 1323 202 1604 483 1604 828 1604 1174 1323 1454 977 1454 632 1454 351 1174 351 828 351 483 632 202 977 202{\\p0}"
		)
		ass:new_event()
		ass:pos(icon_x, icon_y)
		ass:append(
			"{\\rDefault\\an7\\c&HE5E5E5&\\1a&H00&\\bord0\\shad0\\p6}m 895 10 b 401 10 0 410 0 905 0 1399 401 1800 895 1800 1390 1800 1790 1399 1790 905 1790 410 1390 10 895 10 m 895 10 m 925 42 b 1388 42 1763 418 1763 880 1763 1343 1388 1718 925 1718 463 1718 87 1343 87 880 87 418 463 42 925 42{\\p0}"
		)
		ass:new_event()
